defmodule Errors.ThenTest do
  use ExUnit.Case

  describe "then!/1" do
    test "Returns term as success" do
      start = 123
      assert Errors.then!(fn -> start * 2 end) == {:ok, 246}
    end

    test "Returns :ok as :ok" do
      assert Errors.then!(fn -> :ok end) == :ok
    end

    test "Returns success with value as success" do
      start = 123
      assert Errors.then!(fn -> {:ok, start * 2} end) == {:ok, 246}
    end

    test "Returns :error as :error" do
      assert Errors.then!(fn -> :error end) == :error
    end

    test "Returns error with value as error" do
      assert Errors.then!(fn -> {:error, "Test error"} end) == {:error, "Test error"}
    end
  end
  ...
  describe "then!/2" do
    test "only allows result values for first argument" do
      assert_raise ArgumentError,
                   ...
                   fn ->
                     Errors.then!(321, fn _ -> 123 end)
                   end

      assert_raise ArgumentError,
                   "Argument must be {:ok, _} / :ok / {:error, _} / :error, got: :eror",
                   fn ->
                     Errors.then!(:eror, fn _ -> 123 end)
                   end
    end

    test "passes value from {:ok, term} to function" do
      result = {:ok, 10}
      assert Errors.then!(result, fn x -> x * 2 end) == {:ok, 20}
    end

    test "passes nil to function when given :ok" do
      result = :ok
      assert Errors.then!(result, fn nil -> 42 end) == {:ok, 42}
    end

    test "returns :error without calling function" do
      result = :error
      assert Errors.then!(result, fn _ -> raise "Should not be called" end) == :error
    end

    test "returns {:error, term} without calling function" do
      result = {:error, "some error"}

      assert Errors.then!(result, fn _ -> raise "Should not be called" end) ==
               {:error, "some error"}
    end

    test "passes through :ok from function" do
      result = {:ok, 5}
      assert Errors.then!(result, fn 5 -> :ok end) == :ok
    end

    test "passes through :error from function" do
      result = {:ok, 5}
      assert Errors.then!(result, fn 5 -> :error end) == :error
    end

    test "passes through {:error, _} from function" do
      result = {:ok, 5}
      assert Errors.then!(result, fn _ -> {:error, "failed"} end) == {:error, "failed"}
    end

    test "chains multiple then! calls" do
      result =
        {:ok, 10}
        |> Errors.then!(fn x -> x + 5 end)
        |> Errors.then!(fn x -> x * 2 end)
        |> Errors.then!(fn x -> x - 10 end)
    ...
    test "stops chain on error" do
      result =
        {:ok, 10}
        |> Errors.then!(fn x -> x + 5 end)
        |> Errors.then!(fn 15 -> {:error, "oops"} end)
    ...
    test "does not catch exceptions" do
      assert_raise RuntimeError, "The raised error", fn ->
        {:ok, 10}
        |> Errors.then!(fn x -> x + 5 end)
        |> Errors.then!(fn _ -> raise "The raised error" end)
        |> Errors.then!(fn _ -> raise "Should not be called" end)
      end
    end
    ...
    test "handles :ok in chain" do
      result =
        {:ok, 10}
        |> Errors.then!(fn _ -> :ok end)
        |> Errors.then!(fn nil -> 42 end)
  ...
  describe "then/1" do
    ...
    test "An exception is raised" do
      {:error, %Errors.WrappedError{} = wrapped_error} =
        Errors.then(fn -> raise "boom" end)

      assert wrapped_error.message =~
               ~r<\*\* \(RuntimeError\) boom\n    \[CONTEXT\] test/errors/then_test\.exs:\d+: Errors\.ThenTest\.-test then/1 An exception is raised/1-fun-0-/1>
  ...
  describe "then/2" do
    ...
    test "catches exceptions and wraps them in WrappedError" do
      ...
      {:error, %Errors.WrappedError{} = wrapped_error} =
        Errors.then(result, fn _ -> raise "boom" end)
    ...
    test "catches exception and stops chain" do
      result =
        {:ok, 10}
        |> Errors.then(fn x -> x + 5 end)
        |> Errors.then(fn _ -> raise "boom" end)
        |> Errors.then(fn _ -> raise "Should not be called" end)
    ...
    test "catches ArgumentError" do
      result = Errors.then({:ok, "test"}, fn _ -> raise ArgumentError, "invalid argument" end)
    ...
    test "catches custom exceptions" do
      ...
      result = Errors.then({:ok, 5}, fn _ -> raise CustomError end)
